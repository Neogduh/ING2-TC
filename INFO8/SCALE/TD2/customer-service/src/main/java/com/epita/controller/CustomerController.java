...
public class CustomerController {
	...
	public Response getCustomerPurchaseById(@HeaderParam("X-user-id") UUID userId, @PathParam("purchaseId") UUID purchaseId) {
		if (userId == null) {
			return Response.status(Response.Status.BAD_REQUEST).entity("Missing X-user-id header").build();
		}

		Optional<PurchaseReceipt> receipt = service.getPurchaseById(purchaseId);

		if (receipt.isPresent() && receipt.get().getUserId().equals(userId))
			return Response.ok(receipt.get().getProducts()).build();
